...
typedef enum selectTraceType_s {
	SEL_CENTER,				
	SEL_CONTOUR,
 	SEL_RANDOM_VOLUME,		
   	SEL_RANDOM_SURFACE,	
...
typedef struct cgame_s {
	/*int		numScores;
	int 		teamScores[2];
	eth32score_t	scores[MAX_CLIENTS];*/
	//score_t		scores[MAX_CLIENTS];
...
typedef struct settings_s {
	...
	// First Human Aimbot (Half-Logins) by Blackskyliner 04.03.09
	float 	human1_speed;
	// Second Human Aimbot (Full-Logins) by Blackskyliner 04.03.09
	float 	human2_aimX;
	float 	human2_aimY;
	...
	bool 	specLockRemove;	//Removes Speclock
