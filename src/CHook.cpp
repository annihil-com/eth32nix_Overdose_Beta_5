...
static void CG_ParseScore( team_t team ) {
	...
	if( team == TEAM_AXIS ) {
		eth32.ocg.numScores = 0;
		
		eth32.ocg.teamScores[0] = atoi( Syscall.CG_Argv( 1 ) );
		eth32.ocg.teamScores[1] = atoi( Syscall.CG_Argv( 2 ) );
	...
	for(j = 0; j < (eth32.cg.numScores); j++) {
		...
		eth32.ocg.scores[i].score = atoi(			Syscall.CG_Argv( offset + 1 + (j*7)));
		eth32.ocg.scores[i].ping = atoi(			Syscall.CG_Argv( offset + 2 + (j*7)));
		eth32.ocg.scores[i].time = atoi(			Syscall.CG_Argv( offset + 3 + (j*7)));
		powerups = atoi(					Syscall.CG_Argv( offset + 4 + (j*7)));
		eth32.ocg.scores[i].playerClass = atoi(			Syscall.CG_Argv( offset + 5 + (j*7)));
		eth32.ocg.scores[i].respawnsLeft = atoi(		Syscall.CG_Argv( offset + 6 + (j*7)));

		if ( eth32.ocg.scores[i].client < 0 || eth32.ocg.scores[i].client >= MAX_CLIENTS ) {
			eth32.ocg.scores[i].client = 0;
		}

		eth32.cgs.clientinfo[ eth32.ocg.scores[i].client ].score = eth32.ocg.scores[i].score;
		eth32.cgs.clientinfo[ eth32.ocg.scores[i].client ].powerups = powerups;

		eth32.ocg.scores[i].team = eth32.cgs.clientinfo[eth32.ocg.scores[i].client].team;
...
int CDECL eth32_CG_syscall(int command, ...)
{
	...
	switch (command)
	{
		...
		case CG_GETSNAPSHOT:
		{
			...
			/*Alexplay: Used to print the target's ping.
			i = Player order in the scoreboard (not the real slot)
			target = Crosshair target 
			slot = Real slot translated from the scoreboard
			ping = Ping, printed based on the scoreboard*/
			
			/*int slot, ping;
			for (int i=0; i<(eth32.cg.numScores); i++) {
				slot = eth32.ocg.scores[i].client;
				ping = eth32.ocg.scores[i].ping;					
				printf("Player %i ping is %i\n",slot,ping);
			}*/
			
			//Alexplay: We need to update the scores as often as possible so the target prediction (ping based) adjusts in real time.
			if (eth32.cgMod->type != MOD_ETPRO) {
				static int lastUpdateTime = 0;
				if (eth32.cg.cgTime - lastUpdateTime >= 3000) {	//Each 3 second so we don't overload our connection.
					if (!eth32.ocg.demoPlayback && eth32.ocg.mvTotalClients < 1) 
...
{
	if(eth32.settings.drawHackVisuals) return;
