...
void CEngine::RegisterMedia()
{
	...
	eth32.cg.media.grenTarget = Syscall.R_RegisterShaderNoMip("gfx/2d/target_arrow");
...
int CEngine::CG_DrawActiveFrame(int serverTime, int stereoView, int demoPlayback)
{
	...
	if (eth32.settings.specLockRemove) 
...
//		Syscall.R_AddLightToScene(end, 300.0f, 5.0, 1.0, 0.0, 0.0, 0, REF_FORCE_DLIGHT);
	...
	if (pDrawPlayer && (re->reType != RT_SPRITE)) { // this is part of a player and isn't an icon above it's head
		...
		if (drawWeapon) {
			if (eth32.settings.chamsOurWeapon && eth32.settings.chamsWeapon) {
				...
				if  (drawWeapon) {
					if (eth32.settings.outchamweapon && eth32.settings.weapoutChams) {
...
ON_VISUALS re->shaderRGBA[3] = 0xff;
		...
		orig_syscall(CG_R_ADDREFENTITYTOSCENE, re);
  		...
  		if (drawWeapon && eth32.settings.outchamweapon && eth32.settings.weapoutChams) {
                	ON_VISUALS re->customShader = eth32.cg.media.chamsOut[eth32.settings.weapoutChams];
