...
void xcmd_Namesteal()
{
	...
	//Alexplay: no need to buffer the command so we can use it while not connected to a server
...
//Alexplay: Based on eth_randname's code.
void xcmd_RandGuid()
{
	if (!Tools.nGuids) {
		Syscall.UI_Print("^3No guids loaded!\n");
...
{	// sol: added simple spam, feel free to modify however you like
	orig_syscall(UI_CMD_EXECUTETEXT, EXEC_APPEND, "say \"^1[^0ETH32^nnix ^1- ^0OVERDOSE^1] ^n By ^1*^nnixCoders ^0& ^nrabbnix\n\" \n");
	orig_syscall(UI_CMD_EXECUTETEXT, EXEC_APPEND, "say \"^1[^nAn Enemy Territory Hack for Linux^1]\n\" \n");
	orig_syscall(UI_CMD_EXECUTETEXT, EXEC_APPEND, "say \"^1[^nCheat Undetected ^0- ^nCheat FREELY ^0- ^nCheat OpenSource^1]\n\" \n");
}
...
void CXCommands::RegisterCommands()
{
	...
	//orig_Cmd_AddCommand("eth_swag", &swag);
	...
	orig_Cmd_AddCommand("eth_randclguid", &xcmd_RandClGuid);
