...
const char *selectTraceTypeText[SEL_MAX] =
{
	"Center",
	"Contour",
	"Random Volume",
...
{	
	...
	//TODO: Add friendly and targetingUs check	
	if (player->clientNum == attacker) 
...
/*	for (int i=0; i<MAX_CLIENTS; i++) {
		int attacker = eth32.cg.snap->ps.persistant[PERS_ATTACKER];
		int slot = eth32.cg.players[i].clientNum;*/
...
bool CAimbot::traceBox( vec3_t boxOrigin, float dist, vec3_t size, vec3_t trOrigin, int skipEnt, player_t *player, boxtrace_t trType,  vec3_t visible, int maxTraces )
{
	...
	if (eth32.settings.predictionType == PRED_BASIC) {
        ...
        } else if (eth32.settings.predictionType == PRED_SNAP) {
        	VectorMA(boxOrigin, eth32.settings.predTarget, player->currentState->pos.trDelta, boxOrigin);
		VectorMA(boxOrigin,  (eth32.cg.snap->ping/2) * 0.00005f, player->currentState->pos.trDelta, boxOrigin);
		VectorMA(boxOrigin, (eth32.cg.snap->ping/2)  * 0.00005f, player->currentState->pos.trDelta, player->headPt);
	} else if (eth32.settings.predictionType == PRED_MULTI) {
		VectorMA(boxOrigin,  eth32.settings.predTarget, player->currentState->pos.trDelta, boxOrigin);
	...
	} else if (eth32.settings.predictionType == PRED_MIX) {
		VectorMA(boxOrigin, eth32.settings.predTarget, player->currentState->pos.trDelta, boxOrigin);
...
//----------------------------------------------------------------------------------
//                		AUTO TARGET PREDICT
// Alexplay: Target prediction based on ping.
// i = Player order in the scoreboard (not the real slot)
// target = Crosshair target 
// slot = Real slot translated from the scoreboard
// ping = Ping, based on the scoreboard
//----------------------------------------------------------------------------------	
	
	if (eth32.settings.autocorrections) {			
		if (eth32.cgMod->type != MOD_ETPRO) {	//Pings are read different in ETPro
			int target = eth32.cg.xhairClientNum;
			int slot, ping;
			for (int i=0; i<(eth32.cg.numScores); i++) {
				slot = eth32.ocg.scores[i].client;
				ping = eth32.ocg.scores[i].ping;
				if (slot == target) {
					if (ping == 0)
						eth32.settings.predTarget = eth32.settings.predbot;	//Bot prediction
					else if (ping > 0)
						eth32.settings.predTarget = eth32.settings.pred;	//Regular prediction
				}
			}
		} else
 ...
 //----------------------------------------------------------------------------------
//                		AUTO PREAIM/SHOOT TIME
//----------------------------------------------------------------------------------

		if (eth32.settings.preAim && eth32.settings.preShoot && eth32.settings.autoPreAimShoot) {
			eth32.settings.preAimTime = eth32.cg.snap->ping/2;
			eth32.settings.preShootTime = eth32.cg.snap->ping/2;
		}
 	
//----------------------------------------------------------------------------------
//                AUTO DYNAMIC HITBOX BASED ON OUR PING 
//----------------------------------------------------------------------------------
...
//----------------------------------------------------------------------------------
//                AUTO TRACE TYPE BASED ON DISTANCE 
//----------------------------------------------------------------------------------

		if (eth32.settings.selectTraceType == SEL_CENTER) {
			eth32.settings.traceType = TRACE_CENTER;
		} else if (eth32.settings.selectTraceType == SEL_CONTOUR) {
			if (player->distance < 2000) { eth32.settings.traceType = TRACE_CONTOUR; }
			else { eth32.settings.traceType = TRACE_CENTER; }
		} else if  (eth32.settings.selectTraceType == SEL_RANDOM_VOLUME) {
			if (player->distance < 2000) { eth32.settings.traceType = TRACE_RANDOM_VOLUME; }
			else { eth32.settings.traceType = TRACE_CENTER; }
		} else if  (eth32.settings.selectTraceType == SEL_RANDOM_SURFACE) {
			if (player->distance < 2000) { eth32.settings.traceType = TRACE_RANDOM_SURFACE; }
			else { eth32.settings.traceType = TRACE_CENTER; }
		} else if  (eth32.settings.selectTraceType == SEL_CAPSULE_VOLUME) {
			if (player->distance < 2000) { eth32.settings.traceType = SEL_CAPSULE_VOLUME; }
			else { eth32.settings.traceType = TRACE_CENTER; }
		} else if  (eth32.settings.selectTraceType == SEL_CAPSULE_SURFACE) {
			if (player->distance < 2000) { eth32.settings.traceType = SEL_CAPSULE_SURFACE; }
			else { eth32.settings.traceType = TRACE_CENTER; }
		}
...
//----------------------------------------------------------------------------------
			...
			eth32.settings.headbody = HEAD_ONLY;
			
		//Aim at body if we're using the MG42
		//if (BG_PlayerMounted(eth32.cg.snap->ps.eFlags) || (eth32.cg.snap->ps.weapon == WP_MOBILE_MG42) || (eth32.cg.snap->ps.weapon == WP_MOBILE_MG42_SET))
...
//----------------------------------------------------------------------------------
			...
			if (player->distance > 1999) 
				eth32.cg.currentWeapon->bodyTraces = 0;
			eth32.cg.currentWeapon->bodyTraces = ((pointsclose + (pointsfar-pointsclose)*f)/2);
		}

//----------------------------------------------------------------------------------
//                AUTO HEAD HITBOX SIZE AND VECS BASED ON DISTANCE
//----------------------------------------------------------------------------------
...
int SortDistHP(const void *p1, const void *p2) //Alexplay
{
	const player_t *player1 = *(const player_t**)p1;
    	const player_t *player2 = *(const player_t**)p2;
    	player_t *player;
...
void CAimbot::DoBulletBot(void)
{
	...
	if (target) {
		...
		if (isSingleShootWeapon()) {
			...
			if  (eth32.cg.cgTime - lastShotTime >= 10) {
				Autofire(true);
